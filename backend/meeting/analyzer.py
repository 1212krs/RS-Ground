...
_LEAF = {  # 3단계: 세부 노드 (용어면 term=true → 클릭 시 설명 매칭)
    "type": "object",
    "properties": {
        "label": {"type": "string"},
        "term": {"type": "boolean"},
...
3) mindmap: 회의 내용을 마인드맵으로. 반드시 3단계입니다.
   - 중심(label)은 회의의 큰 주제.
   - 가지(children)는 주요 분야/안건 (보통 3~7개).
   - 세부(가지의 children)는 각 안건의 구체 항목.
   - 세부 항목이 '전문 용어나 낯선 개념'이면 term=true, 일반 내용이면 term=false 로 표시하세요.
4) terms: mindmap에서 term=true로 표시한 것들에 대해, 회의 맥락에 맞는 쉬운 설명(2~4문장). label과 term 문자열이 정확히 일치해야 합니다.
