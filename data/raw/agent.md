...
## 7. 다음 단계: RAG 파이프라인 (2026-07-07 결정, PRD 작성 완료 · 구현 미착수)

사용자가 RAG(문서 검색 기반) 기능을 다음 작업으로 결정. 설계 검토를 거쳐 **[docs/PRD-RAG.md](docs/PRD-RAG.md)** 에 상세 요구사항을 확정함. 구현 시 반드시 PRD를 먼저 읽을 것. 핵심 결정 요약:
...
- **형태**: 화면과 분리된 독립 Python 파이프라인(`backend/rag/`)으로 시작. FastAPI 서버화·React 연동은 품질 검증 후 별도 단계.
- **캐시/멱등성**: 임베딩 캐시(SQLite, `sha256(모델명+텍스트)` 키), ChromaDB ID는 `{source}::{chunk_index}` 결정적 ID + upsert, 중간 산출물(chunks.jsonl, umap_coords.json)을 파일로 남김.
- **범위 제외**: PDF/HWPX 파싱, LLM 답변 생성, 서버/화면 연동은 이번 단계 아님.
- **미결정(구현 시작 시 사용자 확인)**: 초기 투입 문서, Python 실행 환경(venv/버전).
- **주의**: `.env`(UPSTAGE_API_KEY), `data/`, `output/`은 `.gitignore` 필수.

### 그 외 후보 (보류 중)

- 사이드바 항목 중 하나를 골라 실제 화면으로 전환(추천 순서: 지식 → AI 채팅 → 보고서 → 에이전트 → 일정/할 일/메모 개별 화면 → 설정) — RAG가 지식/AI 채팅 탭의 기반이 되므로 RAG 이후 자연스럽게 이어짐.
