...
- **최초 작성**: 2026-07-05
- **최종 업데이트**: 2026-08-07 (보고서 탭에 **서식 자동 등록** 기능 추가 — 표시어만 적은 hwpx를 올리면 서식으로 변환. 직전: 2026-08-04 지식 탭 문서 삭제 기능, 2026-08-03 「파주시 예산안 편성지침」 배치 + hwpx 표 추출 버그 수정)
...
backend/
  ...
  report/                         보고서 생성 패키지 (2026-07-08 추가) — engine.py(hwpx 마커 치환·조립, 표준 라이브러리만),
                                  ...
                                  마커 규약(2026-07-17 개정): 필수={{TITLE}}, ○ {{ITEM}}, - {{SUB}} / 선택={{OVERVIEW}}, "1. {{SEC}}"(AI가
                                  내용을 분석해 도출하는 번호 목차 — 번호는 엔진이 순서대로 자동 부여), □ {{HEAD}}(대항목 단계),
...
- **보고서 탭(`/reports`, 2026-07-08 추가)**: 서식(계획보고서/결과보고서/업무보고/보도자료/개인보고서) 선택 → 제목·간단한 내용 입력 + 참고 파일(txt/md/csv/hwp/hwpx/docx, 최대 3개) 첨부 → Claude가 서식 목차에 맞춰 본문 JSON 생성(개조식, 서식별 작성 지침 자동 반영) → 섹션별 편집·표 위치 조정·미리보기 → 진짜 `.hwpx` 파일 다운로드(한글에서 열림). API 키 없으면 대체 생성기로 흐름 유지. 서식 추가는 `backend/report/templates/`에 마커 규약을 지킨 hwpx(+지침 md) 파일만 넣으면 됨 — 코드 수정 불필요. **개인보고서 서식(2026-07-17 추가, 당일 개정)**: 미니멀 서식(개요 상자·표 없음, 단일 '본문' 섹션). 정리 안 된 글을 통째로 넣으면 AI가 **내용을 분석해 핵심 목차(1., 2., ...)를 직접 도출**하고 각 목차 아래를 □(대항목, 선택)/○(핵심)/-(부연)로 구체화 — 목차 번호는 엔진이 자동 부여(sec 레벨). 이 서식은 '2~4블록' 분량 제한이 해제되어 던진 의견·사실을 누락 없이 담는다(max_tokens 16000, brief 한도 12000자). 이를 위해 엔진이 선택 마커(features) 체계와 sec·head 레벨을 지원하게 됨(엔진·컴포저·프론트 오프라인 검증 완료, 기존 4개 서식 회귀 통과 — 기존 서식에선 "1. " 시작 줄이 예전처럼 ○ 항목으로 유지됨). **엔진(조립·추출·프롬프트)은 업무 PC에서 오프라인 검증 완료, FastAPI 라우터 실행 검증은 개발 PC에서 필요** (venv에서 `uvicorn main:app` 후 탭 전체 흐름 확인).
  - **서식 자동 등록(2026-08-07 추가)**: 담당자가 한글에서 만든 문서에 `제목 개요 목차 대항목 대제목 소제목 표제목 머리글 표내용` 표시어를 **한 줄에 하나씩 단독으로** 적어 올리면(문단 전체가 딱 그 낱말일 때만 인식하므로 '추진 내용' 같은 본문 줄은 안 건드린다. 본문 낱말과 헷갈리면 `[[제목]]`처럼 감싸도 되며, 감싼 표시어가 하나라도 있으면 감싼 것만 인정한다) `backend/report/template_builder.py`가 엔진 마커로 자동 변환해 서식으로 등록한다(원본의 글꼴·여백·표 디자인 그대로 유지). 3단계 검사 — ①파일(DRM/형식/손상) ②변환·구조(표시어 인식, 목차 수 = 견본 수, 표 4열 3종세트) ③**시험 생성**(진짜 엔진으로 문서를 한 번 만들어 보고 성공해야 등록). 실패 시 고칠 곳을 한국어 문장으로 돌려준다. 한글이 한 줄을 여러 조각으로 쪼개 저장해도 문단 단위로 이어붙여 인식하며, 견본은 걷어내 한 덩어리로 재삽입하므로 견본 사이 문장이 조용히 삭제되던 조립 특성이 차단된다. 견본 문서: [docs/서식만들기-견본.hwpx](docs/서식만들기-견본.hwpx). API: `POST /api/report/templates/check`(검사만) · `POST /api/report/templates`(등록) · `DELETE /api/report/templates/{id}`.
...
- 2026-07-29 (이어서): **Railway 이전 완료.** 사용자가 대시보드에서 프로젝트 생성·GitHub 연결·Root Directory(`backend`)·Volume(`/data`)·환경변수(`ANTHROPIC_API_KEY`/`UPSTAGE_API_KEY`/`RSG_DATA_DIR`)·도메인 생성(`rs-ground-production.up.railway.app`)까지 진행. 이어서 에이전트가 Railway CLI로 재색인·계정 생성을 대행: **① CLI 설치**(`npm install -g @railway/cli`) **② `railway login`**(브라우저 인증) **③ `railway link --project vibrant-elegance`**(비대화 모드라 `--project` 명시 필요). **④ 재색인 시행착오**: `railway run`은 원격이 아니라 **로컬 PC에서 Railway 환경변수만 주입해 실행**하는 명령이라 `RSG_DATA_DIR=/data`(리눅스 경로)를 로컬 Windows에서 못 씀 → **`railway ssh`(실제 배포 컨테이너 접속)** 로 전환. SSH 키가 없어 `ssh-keygen`으로 생성 + `railway ssh keys add`로 등록 + `ssh-keyscan`으로 `ssh.railway.com` 호스트키를 `known_hosts`에 미리 추가(안 하면 `Host key verification failed`). **⑤ 원본 문서 폴더 부재 발견**: Root Directory를 `backend`로 지정하면 Railway가 **그 하위 폴더만** 컨테이너(`/app`)에 넣고 저장소 루트의 형제 폴더(`data/raw`)는 아예 포함하지 않음(Render의 `rootDir`는 전체 저장소를 클론하고 작업 디렉터리만 바꾸는 방식이라 이 문제가 없었음 — Railway와의 실질적 차이). 다행히 `rag/config.py`의 `RAW_DOCS_DIR = PROJECT_ROOT/"data"/"raw"` 계산이 이 환경에서 `BACKEND_DIR.parent`가 `/`로 잡혀 우연히 `/data/raw`(우리가 만든 Volume 경로)와 일치 → 로컬 `data/raw`(1.7MB, 5개 파일)를 `tar czf - -C ../data raw | railway ssh -- "cd /data && tar xzf -"`로 볼륨에 직접 업로드해 해결(재배포해도 Volume이라 유지됨). **⑥ 재색인 성공**: `railway ssh -- python -m rag.pipeline index` → 1,807조각(회계 1,046+예산 710+프로젝트 51), Render 시절과 동일 규모로 확인. **⑦ 계정 생성**: `railway ssh -- python -m auth.manage add rsakim <비번> rsakim`. 첫 시도 후 curl로 로그인 검증 시 필드명을 `username`으로 잘못 보내 401이 났는데, 실제 원인은 이 오타였음(`auth/api.py`는 `{loginId, password}`를 받음) — 확인 과정에서 비밀번호를 base64 경유로 한 번 더 안전하게 재설정(`railway ssh`가 `python -c "..."` 형태의 복잡한 인용을 원격 셸로 그대로 못 넘겨 `syntax error`가 났던 것도 발견 → 스크립트 파일을 만들어 `cat | railway ssh -- "cat > /app/x.py && python x.py"`로 전송·실행하는 방식으로 우회. 이때도 스크립트를 `/tmp`에 두면 `sys.path`에 `/app`이 없어 `from auth import store`가 실패하므로 반드시 `/app`에 둬야 함). **⑧ `FRONTEND_ORIGINS`**: 사용자가 알려준 실제 Vercel 주소(`rs-ground.vercel.app`)로 `railway variables --set`. **최종 검증**: curl로 로그인(200, 토큰 발급)·`/api/rag/documents`(문서 5종·1,807조각 정상 조회) 확인, 실제 웹(`rs-ground.vercel.app`)에서 `rsakim` 로그인 성공 확인(Vercel `VITE_API_BASE`도 사용자가 갱신·재배포). **미완료/참고**: Render 서비스는 이중 과금 방지를 위해 별도로 정지·삭제 필요(사용자 몫, 접속 가능할 때). Railway 무료 크레딧("30 days or $5")이 소진되면 결제수단 등록 필요. AI 채팅(Claude 답변 자체)의 실제 동작은 문서 목록 조회까지만 확인했고 답변 생성까지는 미확인.
- 2026-07-29: **백엔드 배포처 Render → Railway 전환(설정만, 실제 대시보드 이전은 사용자 몫).** 회사망에서 Render 대시보드 로그인 자체가 안 되는 문제 발견 → Railway로 옮기기로 결정. 저장소에 `backend/railway.json`(build/start 명령, Nixpacks)·`backend/.python-version`(3.11.9 고정, Render의 `PYTHON_VERSION` env var 대체) 신규 추가. 기존 `render.yaml`은 사용자 선택으로 **삭제하지 않고 보관**(당장 안 쓰지만 만약을 대비). `docs/DEPLOY.md` 3~8절을 Railway 절차로 재작성(Root Directory=`backend` 지정 필요, 영구 디스크→Volume(`/data`), Shell 대신 Railway CLI(`railway run`)로 재색인·계정 생성). `CLAUDE.md`의 "Deployed" 한 줄도 갱신. **미완료**: 실제 Railway 프로젝트 생성·GitHub 연결·환경변수 입력·Volume 생성·`RSG_ALLOWED_HOSTS`/`FRONTEND_ORIGINS`/`VITE_API_BASE` 값 갱신은 대시보드 작업이라 에이전트가 대신할 수 없음 — `docs/DEPLOY.md` 3절 그대로 따라가면 됨. 계정 DB·벡터DB는 Render 볼륨에서 자동 이전되지 않으므로 Railway에서 재색인(`rag.pipeline index`)·계정 재생성(`auth.manage add`) 필요.
