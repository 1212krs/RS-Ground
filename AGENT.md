...
- **지식 탭(문서 업로드)을 쓰려면 RAG API 서버도 따로 띄워야 한다.** (2026-07-07 추가) `npm run dev`만으로는 안 되고, 별도 터미널에서:
  ```powershell
  cd backend
  ./venv/Scripts/python.exe -m uvicorn rag.api:app --reload --port 8000
  ```
  Vite dev 서버가 `/api/rag/*` 요청을 이 서버(8000번 포트)로 프록시한다(`vite.config.js`의 `server.proxy`). 이 서버가 안 떠 있으면 지식 탭에서 업로드/목록 조회가 실패한다.
...
backend/
  rag/api.py                      RAG용 FastAPI 서버. POST/GET /api/rag/documents (업로드 시 전체 재색인). uvicorn으로 별도 실행 필요 — 3장 참고.
```

- **라우트 구성**(`App.jsx`): `/login`(공개) · `/`(보호, `DashboardPage`) · `/knowledge`(보호, `KnowledgePage`) · `PENDING_NAV`에 나열된 경로(`/chat`, `/reports`, `/agents`, `/calendar`, `/todos`, `/memos`, `/settings`, 전부 보호)는 `ComingSoonPage`로 연결 · `*`는 `NotFoundPage`.
- **다크 배경 페이지**: `AppLayout.jsx`의 `NO_TOPBAR_PATHS`(`/`, `/agents`, `/knowledge`)에 경로를 넣으면 상단바가 사라지고 `.main-area.main-dark`(검정 배경)가 적용된다. 각 페이지는 `.oa`(대시보드)/`.kb`(지식) 처럼 자기 스코프에 다크 팔레트 CSS 변수를 정의하는 컨벤션(로그인 페이지의 `.nx`와 동일 패턴).
